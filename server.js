...
// Connect to MongoDB
mongoose.connect('mongodb://localhost/ipMonitor', { useNewUrlParser: true, useUnifiedTopology: true });
...
app.post('/api/ip', async (req, res) => {
    ...
    const newIP = new IP({ address,location, status: 'Unknown', downtime: [] });
    await newIP.save();
    res.send(newIP);
});
...
app.listen(9991, () => {
    console.log('Server is running on http://localhost:9991');
});
